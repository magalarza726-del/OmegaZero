...
const asset=join(repo,'assets','v3.1.0-20260815173000');
...
test('main.js es composition root y las áreas están modularizadas',()=>{
  ...
  for(const module of ['appChrome','board','play','learn','library','pawnGallery','structureStudy','tcom','transform']){
...
test('inicio expone Jugar, Aprender e Investigar',()=>{
  ...
  for(const destination of ['setup','analysis','strategy','library','stockfishTransform','tcomLab','structureStudy']) assert.ok(chrome.includes(`data-go=\"${destination}\"`),destination);
});

test('la página de producción apunta solo al build v3',()=>{
  const html=readFileSync(join(repo,'index.html'),'utf8');
  assert.ok(html.includes('v3.1.0-20260815173000'));
