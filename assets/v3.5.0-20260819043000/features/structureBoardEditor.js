 ...
 bindStructureBoardEditor(){
  ...
  $$('[data-editor-square]',board).forEach(cell=>{
   cell.onpointerdown=event=>{
    ...
    if(event.button!==0)return;const existing=lab.pieces.find(piece=>piece.square===square);lab.drag={id:event.pointerId,from:square,x:event.clientX,y:event.clientY,moved:false,existing:existing?{...existing}:null};cell.setPointerCapture?.(event.pointerId);
   };
   cell.onpointermove=event=>{const drag=lab.drag;if(!drag||drag.id!==event.pointerId)return;if(Math.hypot(event.clientX-drag.x,event.clientY-drag.y)>6)drag.moved=true};
   cell.onpointerup=event=>{
    const drag=lab.drag;if(!drag||drag.id!==event.pointerId)return;lab.drag=null;
    ...
    else place(drag.from);
    this.renderStructureBoardEditor();
   };
   cell.onpointercancel=()=>{lab.drag=null};
  });
 }
};
