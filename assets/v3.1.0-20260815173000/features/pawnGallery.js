...
export const pawnGalleryMethods = {
 ...
 renderPawnGallery(){
  ...
  <section class="pawn-gallery-method panel"><div class="pawn-color-legend"><span><i class="legend-blue"></i><b>Azul</b> detrás de cada peón</span><span><i class="legend-green"></i><b>Verde</b> casillas atacadas / de apoyo</span><span><i class="legend-orange"></i><b>Naranja</b> por delante y sin control propio</span><span data-purple-legend><i class="legend-purple"></i><b>Morado</b> archivo sin peón propio (estado 9)</span><span><i class="legend-double">2×</i><b>2×</b> doble control</span></div><p>Una familia no borra la precisión del código: dentro de ella puedes abrir hasta diez ejemplos y luego inspeccionar cualquier ejemplo en el tablero ampliado con las anotaciones de OmegaZero.</p></section>
  <section class="pawn-gallery-toolbar"><label class="pawn-gallery-search-label">Buscar familia o código<input data-pawn-gallery-search autocomplete="off" spellcheck="false" placeholder="Ej. P-014, 1239, sube" value="${esc(savedFilter)}"></label><label class="pawn-gallery-purple-toggle"><input type="checkbox" data-pawn-gallery-purple ${showOpenFiles?'checked':''}><span>Pintar archivos sin peón (morado)</span></label><span data-pawn-gallery-count>${families.length} familias</span><button type="button" data-pawn-gallery-clear>Mostrar todas</button></section>
  <section class="structure-family-list pawn-family-list">${families.map(family=>`<details class="structure-family-row pawn-family-row" data-pawn-family="${family.id}" data-family-key="${family.key}" data-search="${esc(`${family.id} ${family.key} ${family.label} ${family.representative} ${family.examples.join(' ')}`.toLowerCase())}"><summary><div><small>${family.id}</small><b>${esc(family.label)}</b><code>${family.key}</code></div><span>${family.representative}</span></summary><div class="structure-family-lazy" data-pawn-family-body></div></details>`).join('')}</section>`;
  ...
  purple?.addEventListener('change',event=>{
   this.pawnGalleryShowOpenFiles=event.target.checked;this.db.settings.pawnGalleryShowOpenFiles=event.target.checked;saveDb(this.db);
   v.classList.toggle('pawn-gallery-hide-open',!event.target.checked);$('[data-purple-legend]')?.classList.toggle('disabled',!event.target.checked);
  });
  ...
  $$('.pawn-family-row').forEach(row=>row.addEventListener('toggle',()=>{
   if(!row.open)return;const body=$('[data-pawn-family-body]',row);if(!body||body.dataset.loaded==='1')return;
