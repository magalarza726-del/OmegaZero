import { APP_VERSION, $, $$, esc, publicAsset, getStructureFamilies, analyzePieceConfiguration, formatGradient2D } from '../app/deps.js';
...
export const structureStudyMethods={
 renderStructureStudy(){
  const v=$('#view');
  v.innerHTML=`<section class="page-head structure-study-head"><button data-structures-home aria-label="Volver">←</button><div><small>INVESTIGAR · TAXONOMÍA GEOMÉTRICA</small><h1>Estudiar estructuras</h1><p>OmegaZero separa la posición en alfabetos pequeños. Cada módulo agrupa posiciones por su geometría y gradiente; las configuraciones exactas aparecen únicamente como ejemplos de una familia.</p></div></section>
  <section class="structure-study-intro panel"><div><b>Color analítico común</b><p><span class="structure-color-chip front"></span><strong>Verde claro</strong>: control hacia delante. <span class="structure-color-chip back"></span><strong>Azul</strong>: región o control hacia atrás. El tablero siempre utiliza el color seleccionado en Configuración.</p></div><small>La agrupación no califica una familia como buena o mala: solo describe relaciones geométricas reproducibles.</small></section>
 ...
 pieceStructureBoardHtml(kind,analysis,compact=false){
  ...
  for(let rank=8;rank>=1;rank-=1){
   for(let file=1;file<=8;file+=1){
    ...
    const piece=pieceBySquare.get(square),front=analysis.frontControl.get(square)||0,back=analysis.backControl.get(square)||0;
    ...
    if(back>0)classes.push('control-back');
    if(front>0)classes.push('control-front');
    if(front>0&&back>0)classes.push('control-both');
    if(piece)classes.push('occupied');
    const count=front+back;
    const asset=piece?publicAsset(`pieces/alpha/w${piece.type}.png`,APP_VERSION):'';
    cells.push(`<span class="${classes.join(' ')}" data-piece-square="${square}" title="${square}${front?` · frente ${front}`:''}${back?` · detrás ${back}`:''}">${piece?`<img src="${asset}" class="piece-structure-piece" alt="${esc(piece.role)} en ${square}" draggable="false">`:''}${count>1?`<b>${count}×</b>`:''}</span>`);
   }
  }
 ...
 renderStructureFamilies(){
  ...
  v.innerHTML=`<section class="page-head structure-families-head"><button data-structure-back aria-label="Volver">←</button><div><small>${copy.eyebrow}</small><h1>${copy.title}</h1><p>${copy.description}</p></div></section><section class="structure-family-method panel"><p><b>Cómo se forma una familia.</b> Se toman las cuatro piezas en su orden fijo y se calcula el gradiente 2D entre cada pareja consecutiva. Para la familia se conserva su dirección (N, NE, E, SE, S, SW, W, NW); el desplazamiento exacto se mantiene dentro de cada ejemplo. El mapa de control es intrínseco a estas cuatro piezas: no presupone peones ni piezas enemigas.</p><div class="structure-family-key"><span><i class="front"></i>Verde claro = control por delante</span><span><i class="back"></i>Azul = control por detrás</span><span>El control lateral se conserva en el análisis, pero no se colorea</span><span>Filas permitidas: <b>${kind==='minor'?'2–5':'1–3'}</b></span></div></section><section class="structure-family-toolbar"><label>Buscar familia, dirección o casilla<input type="search" data-structure-family-search autocomplete="off" placeholder="Ej. NE|S|E, M-001 o c3 f4 g2 f3" value="${esc(query)}"></label><span data-structure-family-count>${families.length} familias</span><button type="button" data-structure-family-clear>Mostrar todas</button></section><section class="structure-family-list">${families.map(family=>`<details class="structure-family-row" data-family-id="${family.id}" data-family-key="${family.key}" data-search="${esc(`${family.id} ${family.key} ${family.label} ${family.representative.join(' ')} ${family.examples.flat().join(' ')}`.toLowerCase())}"><summary><div><small>${family.id}</small><b>${family.label}</b><code>${family.key}</code></div><span>${family.representative.join(' · ')}</span></summary><div class="structure-family-lazy" data-family-body></div></details>`).join('')}</section>`;
  ...
  $$('.structure-family-row').forEach(row=>row.addEventListener('toggle',()=>{
   if(!row.open)return;const body=$('[data-family-body]',row);if(!body||body.dataset.loaded==='1')return;
