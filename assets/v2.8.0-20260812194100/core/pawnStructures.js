...
export function analyzePawnStructure(code) {
  ...
  const { displayFiles, displayRanks, baseRank } = PAWN_STRUCTURE_MODEL;
  const pawns = heights.map((height, index) => ({
    index,
    file: index + 1,
    rank: baseRank + height,
    height,
  }));
  ...
  for (let rank = 1; rank <= displayRanks; rank += 1) {
    for (let file = 0; file < displayFiles; file += 1) {
      ...
      const coreIndex = file - 1;
      const isCore = coreIndex >= 0 && coreIndex < 4;
      const ownPawnRank = isCore ? pawns[coreIndex].rank : null;
      const behind = Boolean(isCore && rank < ownPawnRank);
      const ahead = Boolean(isCore && rank > ownPawnRank);
      ...
      cells.push({ file, rank, pawn, controlCount, behind, aheadUncontrolled, isCore });
    }
  }
