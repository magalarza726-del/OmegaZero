...
class App{
 ...
 renderPawnGallery(){
  ...
  const apply=()=>{const query=(search?.value||'').replace(/[^01239]/g,'').slice(0,4);if(search&&search.value!==query)search.value=query;let visible=0;$$('[data-pawn-structure]').forEach(card=>{const show=!query||card.dataset.code.includes(query);card.hidden=!show;if(show)visible+=1});if(count)count.textContent=`${visible} / ${PAWN_STRUCTURE_CODES.length}`};
  search?.addEventListener('input',apply);
  $('[data-pawn-gallery-clear]')?.addEventListener('click',()=>{if(search)search.value='';apply();search?.focus()});
